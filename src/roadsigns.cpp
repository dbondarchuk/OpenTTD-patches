...
RoadSignDirection GetYieldSignDirection(TileIndex tile)
{
	...
	if (i > ROAD_SIGN_DIRECTION_END) return ROAD_SIGN_DIRECTION_NONE;

	const TileIndex neededNeighbor = AddTileIndexDiffCWrap(tile, neighbors[i]);
	if (IsRoadStopTile(neededNeighbor)
		|| IsRoadBridgeHeadTile(neededNeighbor)
		|| IsRoadTunnelTile(neededNeighbor)) return (RoadSignDirection)i;

	if (neededNeighbor == INVALID_TILE || !IsTileType(neededNeighbor, MP_ROAD)) return ROAD_SIGN_DIRECTION_NONE;
	if (GetRoadTileType(neededNeighbor) == ROAD_TILE_CROSSING) return (RoadSignDirection)i;
	if (GetRoadTileType(neededNeighbor) == ROAD_TILE_NORMAL)
	{
		RoadBits neighborRoad = GetAllRoadBits(neededNeighbor);
		if (!(neighborRoad & rb[3 - i])) return ROAD_SIGN_DIRECTION_NONE;

		DisallowedRoadDirections drd = GetDisallowedRoadDirections(neededNeighbor);
		if (drd == ((i % 2 == 0) ? DRD_SOUTHBOUND : DRD_NORTHBOUND) || drd == DRD_BOTH) return ROAD_SIGN_DIRECTION_NONE;
	}
...
static Vehicle* CheckVehicleHeadingNeededDirection(Vehicle* v, void* data) {
	...
	return v->z_pos > d->z
		|| !IsValidDiagDirection(DirToDiagDir(v->direction))
		|| DirToDiagDir(v->direction) != d->direction
		|| v->cur_speed < (min_vehicle_speed_km_h * 2) + 2
		? nullptr
		: v;
}
...
TrackdirBits GetYieldSignDisallowedDirections(TileIndex tile)
{
	...
	if (direction == ROAD_SIGN_DIRECTION_NONE) return TRACKDIR_BIT_NONE;
	...
	if (CheckVehicleOnTile(tile, tile)
		|| CheckVehicleOnTile(otherTile1, tile)
		|| CheckVehicleOnTile(otherTile2, tile)) {
...
{
	if (CheckIsAnyMovingVehicleOnTile(tile)) return TRACKDIR_BIT_MASK; // If there is any vehicle moving on intersection - stop;
...
CommandCost CmdBuildYieldSign(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, const char* text)
{
	...
	if (!_settings_game.construction.road_signs) return CMD_ERROR; // Sanity check.
...
/* Check for correct location (road). */
	...
	if (Company::IsValidID(_current_company)) {
		...
		if (owner == OWNER_TOWN) {
			if (!_settings_game.construction.allow_building_rs_in_towns) return_cmd_error(STR_ERROR_ROAD_SIGNS_NOT_ALLOWED_ON_TOWN_ROADS);
		}
		else {
	...
	if (CountBits(GetAllRoadBits(tile)) != 3) return_cmd_error(STR_ERROR_CAN_ONLY_BE_PLACED_ON_3WAY_ROAD_JUNCTIONS);
	if (HasTrafficLights(tile)) return_cmd_error(STR_ERROR_ALREADY_BUILT);
	if (HasYieldSign(tile)) return_cmd_error(STR_ERROR_ALREADY_BUILT);
	if (HasStopSign(tile)) return_cmd_error(STR_ERROR_ALREADY_BUILT);

	RoadSignDirection direction = GetYieldSignDirection(tile);
	if (direction == ROAD_SIGN_DIRECTION_NONE) return_cmd_error(STR_ERROR_CAN_ONLY_BE_PLACED_ON_3WAY_ROAD_JUNCTIONS);
...
CommandCost CmdRemoveYieldSign(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, const char* text)
{
	...
	if (!IsTileType(tile, MP_ROAD) || GetRoadTileType(tile) != ROAD_TILE_NORMAL || !HasYieldSign(tile)) return CMD_ERROR;

	/* Check owner, but only if a valid player is executing this command. */
	if (Company::IsValidID(_current_company)) {
		Owner owner = GetTileOwner(tile);
		if (owner == OWNER_TOWN) {
			if (!_settings_game.construction.allow_building_rs_in_towns && !_cheats.magic_bulldozer.value) return_cmd_error(STR_ERROR_ROAD_SIGNS_NOT_ALLOWED_ON_TOWN_ROADS);
		}
		else {
...
{
	/* Check for correct location (road with traffic lights). */
	if (!IsTileType(tile, MP_ROAD) || GetRoadTileType(tile) != ROAD_TILE_NORMAL || !HasStopSign(tile)) return CMD_ERROR;
...
void DrawYieldSign(TileInfo* ti)
{
	...
	if (direction == ROAD_SIGN_DIRECTION_NONE) return;
...
void DrawHydrant(TileInfo* ti) {
	...
	if (oneway == DRD_SOUTHBOUND) rs = 1 - rs; // To avoid situations when it's placed on highway's wrong side
